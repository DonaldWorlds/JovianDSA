...
def occurs_in_the_middle():
    ...
    query = 7
    expected_index = 3
    results = mybinarysearch.locate_card(cards,query)
    
    # Check if the assertion passes
    assert results == expected_index, f"Expected {expected_index}, but got {results}"
    print(f"Test passed: Expected {expected_index}, got {results}")
...
def occurs_in_the_first():
    ...
    print(f"test passed with values expexted as {expected_indexed_output} and {results} as results")
...
def occurs_in_the_last():
    ...
    print(f"Test passed with expected index as {expected_indexed_output} and results as {results} ") 
...
def contains_one():
    ...
    print(f"Test passed with expected {expected_indexed_output} and reults as {results}")
...
def does_not_contain_query():
    ...
    results = mybinarysearch.locate_card(cards,query)
    assert results == expected_indexed_output, f"Expected {expected_indexed_output}, but got {results}" 
    print(f"The Test passed with expected {expected_indexed_output} and results as {results}")
...
def repeated_number_in_cards_two():
    ...
    results = mybinarysearch.locate_cards_three(cards,query)
...
def query_occurs_multiple_times():
    ...
    results = mybinarysearch.locate_card(cards,query)
    assert results == expected_indexed_output, f"Expected {expected_indexed_output}, got {results}" # this will be the function call
    print(f"The test passed with expected {expected_indexed_output} ans results as {results}")
